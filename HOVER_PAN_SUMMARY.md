...
1. **不依赖 transform-origin**：所有平移由容器 `translate3d` 完成，缩放/旋转留在内层，避免 origin 边界导致的压缩。  
2. **对所有尺寸有效**：即使图片比视口小，也能通过虚拟溢出获得平滑过渡，而不会破坏 zoom 模式。  
3. **模块化组合**：HoverLayer 只关注位置百分比；CurrentFrameLayer 专注于转换为像素偏移；StackView 负责提供真实/虚拟尺寸，并在翻页时重置。  
4. **用户可控**：提供显式配置项（最小缓冲百分比、自动放大阈值、拖拽 fallback 开关）。
