...
use super::types::{RealWorldTestResult, TranscodeBenchmarkReport, TranscodeBenchmarkResult};
...
pub async fn run_realworld_benchmark(
        ...
        if file_path.is_file() {
            if let Some(ext) = file_path.extension().and_then(|e| e.to_str()) {
                ...
                if image_exts.contains(&ext_lower.as_str())
                    || archive_exts.contains(&ext_lower.as_str())
                {
    ...
    let avg_time_ms = if total_files > 0 {
        total_time_ms / total_files as f64
    } else {
        0.0
    };
    let throughput = if total_time_ms > 0.0 {
        (total_files as f64 / total_time_ms) * 1000.0
    } else {
        0.0
    };
...
#[command]
pub async fn run_transcode_benchmark(
    file_path: String,
) -> Result<TranscodeBenchmarkReport, String> {
    use std::fs;
...
fn encode_rgba_to_format(
    ...
    let img = RgbaImage::from_raw(width, height, rgba.to_vec()).ok_or("创建图像失败")?;
