...
#[command]
pub async fn convert_avif_to_webp(
    image_path: String,
) -> Result<String, String> {
    use std::fs;
    ...
    let image_data = fs::read(&image_path)
        .map_err(|e| format!("读取 AVIF 文件失败: {}", e))?;

    // 使用 image crate 解码 AVIF
    let img = image::load_from_memory(&image_data)
        .map_err(|e| format!("解码 AVIF 图片失败: {}", e))?;

    // 创建临时文件
    let temp_dir = std::env::temp_dir();
    let original_filename = image_path.file_stem()
        .ok_or("无效的文件名")?
        .to_string_lossy();
    let temp_webp_path = temp_dir.join(format!("neoview_avif_convert_{}.webp", original_filename));

    // 创建文件写入器
    let file = fs::File::create(&temp_webp_path)
        .map_err(|e| format!("创建 WebP 文件失败: {}", e))?;
    ...
    println!("✅ AVIF 转 WebP 完成: {} -> {}", image_path.display(), temp_webp_path.display());
...
/// 保存超分后的图片
#[command]
pub async fn save_upscaled_image(
    file_path: String,
    image_data: Vec<u8>,
) -> Result<(), String> {
    use std::fs;
