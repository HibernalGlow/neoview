...
// 重导出公共类型和常量
pub use types::{
    ArchiveEntry, ArchiveFormat, ArchiveMetadata, CachedImageEntry, ARCHIVE_IMAGE_EXTENSIONS,
    IMAGE_CACHE_LIMIT, RAR_EXTENSIONS, SEVENZ_EXTENSIONS, ZIP_EXTENSIONS,
};

// 重导出工具函数
pub use utils::{
    detect_image_mime_type, encode_jpeg, encode_webp, get_archive_metadata, is_image_file,
    normalize_archive_key, normalize_inner_path, resize_keep_aspect_ratio, zip_datetime_to_unix,
    StreamReader,
};
...
mod property_tests {
    ...
    fn prop_api_backward_compatibility_archive_format_detection() {
        ...
        assert_eq!(
            ArchiveFormat::from_extension(Path::new("test.cbz")),
            ArchiveFormat::Zip
        );
        assert_eq!(
        ...
        assert_eq!(
            ArchiveFormat::from_extension(Path::new("test.cbr")),
            ArchiveFormat::Rar
        );
        assert_eq!(
        ...
        assert_eq!(
            ArchiveFormat::from_extension(Path::new("test.cb7")),
            ArchiveFormat::SevenZ
        );
        assert_eq!(
