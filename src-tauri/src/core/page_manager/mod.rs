...
impl PageContentManager {
    ...
    /// 快速扫描压缩包（只返回前 N 张图片）
    fn scan_archive_quick(&self, path: &str, limit: usize) -> Result<Vec<String>, String> {
        let manager = self
            .archive_manager
            .lock()
            .map_err(|e| format!("获取压缩包管理器锁失败: {}", e))?;

        manager.scan_archive_images_fast(Path::new(path), limit)
    }

    /// 快速打开书籍（先返回前 N 页，用于大压缩包首屏优化）
    pub async fn open_book_quick(
        &mut self,
        path: &str,
        quick_limit: usize,
    ) -> Result<(BookInfo, bool), String> {
        log::info!("📖 PageManager: 快速打开 {} (limit={})", path, quick_limit);
        ...
        let path_obj = Path::new(path);
        if !Self::is_archive_file(path) || path_obj.is_dir() {
            let info = self.open_book(path).await?;
            return Ok((info, false));
        }

        let quick_images = self.scan_archive_quick(path, quick_limit)?;
        if quick_images.is_empty() {
            return Err("压缩包中未找到图片".to_string());
        }

        log::info!("📖 PageManager: 快速扫描找到 {} 页", quick_images.len());
        let book = BookContext::from_archive(path, quick_images);
        let info = BookInfo::from(&book);
        self.current_book = Some(book);
    ...
    pub fn update_book_pages(
        ...
        if book.path != path {
            return Err("书籍路径不匹配".to_string());
        }
        let new_book = BookContext::from_archive(path, all_pages);
        book.pages = new_book.pages;
        book.total_pages = new_book.total_pages;
        log::info!("📖 PageManager: 更新页面列表，共 {} 页", book.total_pages);
        Ok(book.total_pages)
    }

    /// 获取完整页面列表（用于后台扫描）
    pub fn scan_archive_full(&self, path: &str) -> Result<Vec<String>, String> {
