...
impl ThumbnailManager {
    ...
    pub fn ensure_folder_thumbnail(&self, folder_path: &Path, thumbnail_url: &str) -> Result<(), String> {
        ...
        match self.db.find_by_bookpath(&bookpath) {
            ...
            Ok(None) => {
                // 没有缩略图，创建记录指向提供的缩略图
                let thumbnail_path = Path::new(thumbnail_url.strip_prefix("file://").unwrap_or(thumbnail_url));
                
                // 获取缩略图的相对路径
                let thumbnail_relative_path = match thumbnail_path.strip_prefix(&self.db.thumbnail_root) {
                    Ok(p) => p.to_string_lossy(),
                    Err(_) => {
                        // 如果不在 thumbnail_root 下，使用完整路径
                ...
                let record = ThumbnailRecord {
                    ...
                    relative_thumb_path: thumbnail_relative_path.to_string(),
                ...
                self.db.upsert_thumbnail(record)
                    .map_err(|e| format!("保存文件夹缩略图记录失败: {}", e))?;
                
                println!("✅ 为文件夹创建共享缩略图: {} -> {}", folder_path.display(), thumbnail_url);
                Ok(())
            }
            Err(e) => {
                Err(format!("查询文件夹缩略图失败: {}", e))
            }
        }
    }
