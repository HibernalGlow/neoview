...
<div class="space-y-4">
	...
	<div class="space-y-2">
		<Label>翻译服务</Label>
		<div class="bg-muted/30 flex rounded-md border p-1">
			<button
				class="flex flex-1 items-center justify-center gap-1 rounded px-2 py-1.5 text-xs transition-colors {config.type ===
				'disabled'
	...
	{#if config.type === 'libretranslate'}
		...
		<div class="bg-muted/20 space-y-3 rounded-md border p-3">
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-2">
					...
					<div
						...
						title={serviceOnline === null ? '未检测' : serviceOnline ? '服务在线' : '服务离线'}
					>
						{#if checkingStatus}
			...
			<div class="space-y-2 border-t pt-3">
				...
				<div class="flex items-center gap-2">
					<code class="bg-muted flex-1 rounded px-2 py-1.5 font-mono text-xs break-all">
						{startCommand}
					</code>
					<Button
						variant="ghost"
				...
				<p class="text-muted-foreground text-xs">在终端运行此命令启动 LibreTranslate 服务</p>
			</div>
		</div>
		...
		<div class="bg-muted/20 space-y-3 rounded-md border p-3">
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-2">
					...
					<div
						class="flex items-center gap-1 text-xs"
						title={ollamaOnline === null ? '未检测' : ollamaOnline ? '服务在线' : '服务离线'}
					>
			...
			<div class="flex items-center gap-2">
				...
				<select
					class="bg-background h-7 flex-1 rounded border px-2 text-xs"
			...
			<div class="space-y-2">
				...
				<Input value={config.ollamaModel} oninput={updateOllamaModel} placeholder="qwen2.5:7b" />
				<p class="text-muted-foreground text-xs">推荐: qwen2.5:7b, llama3.2:3b</p>
			</div>
			...
			<div class="space-y-2 border-t pt-3">
				...
				<!-- 启动服务 -->
				<div class="flex items-center gap-2">
					<code class="bg-muted flex-1 rounded px-2 py-1.5 font-mono text-xs"> ollama serve </code>
					<Button
						variant="ghost"
						size="sm"
						class="h-8 w-8 shrink-0 p-0"
				...
				<!-- 运行模型 -->
				<div class="flex items-center gap-2">
					<code class="bg-muted flex-1 rounded px-2 py-1.5 font-mono text-xs">
				...
				<p class="text-muted-foreground text-xs">1. 启动服务 2. 运行模型（自动下载）</p>
			</div>
				...
				<div class="space-y-2 border-t pt-3">
					...
					{#if ollamaStats !== null}
						{#if ollamaStats.length === 0}
							<p class="text-muted-foreground text-xs">当前无运行中的模型</p>
						{:else}
							<div class="space-y-1">
						...
						{/if}
					{:else}
						<p class="text-muted-foreground text-xs">点击刷新查看运行状态</p>
					{/if}
				</div>
			{/if}
			...
			<div class="space-y-2">
				...
				<textarea
					class="bg-background min-h-[60px] w-full resize-y rounded border p-2 text-xs"
			...
			<div class="flex items-center justify-between border-t pt-3">
				<div class="flex items-center gap-2">
					<Power class="text-muted-foreground h-3 w-3" />
					<Label class="text-xs">自动启动 Ollama</Label>
				</div>
				<button
					class="h-5 w-9 rounded-full transition-colors {config.ollamaAutoStart
						? 'bg-primary'
						: 'bg-muted'}"
					onclick={() =>
						aiTranslationStore.updateConfig({ ollamaAutoStart: !config.ollamaAutoStart })}
					title="开启后，当翻译需要 Ollama 但服务未运行时自动启动"
				>
					<div
						class="bg-background h-4 w-4 rounded-full shadow transition-transform {config.ollamaAutoStart
							? 'translate-x-4'
							: 'translate-x-0.5'}"
					></div>
				</button>
			</div>
			<p class="text-muted-foreground text-xs">开启后，翻译时若 Ollama 未运行将自动启动服务</p>
		</div>
	{/if}
	...
	{#if config.type !== 'disabled'}
			...
			<div class="space-y-2">
				...
				<div class="bg-muted/30 flex rounded-md border p-1">
					{#each languageOptions as option}
						<button
							class="flex-1 rounded px-2 py-1 text-xs transition-colors {config.sourceLanguage ===
							option.value
								? 'bg-background shadow-sm'
								: 'hover:bg-muted'}"
			...
			<div class="space-y-2">
				...
				<div class="bg-muted/30 flex rounded-md border p-1">
					{#each targetLanguageOptions as option}
						<button
							class="flex-1 rounded px-2 py-1 text-xs transition-colors {config.targetLanguage ===
							option.value
								? 'bg-background shadow-sm'
								: 'hover:bg-muted'}"
		...
		<div class="space-y-2">
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-2">
					<button class="text-primary text-xs hover:underline" onclick={addRule}>
						+ 添加规则
					</button>
				</div>
			</div>
			...
			<div class="space-y-2">
				{#each config.cleanupRules || [] as rule (rule.id)}
					<div class="bg-muted/20 flex items-start gap-2 rounded border p-2">
						<!-- 开关 -->
						<button
							class="mt-1 flex h-4 w-4 items-center justify-center rounded border text-xs {rule.enabled
						...
						<div class="flex-1 space-y-1">
							...
							<input
								type="text"
								class="bg-background w-full rounded border px-2 py-1 font-mono text-xs"
								value={rule.pattern}
								placeholder="正则表达式，如 \\[.*?\\]"
							...
							<div class="flex flex-wrap gap-1">
								<button
									class="rounded border px-1.5 py-0.5 text-[10px] {rule.applyTo.includes('all')
										? 'bg-primary text-primary-foreground'
								...
								{#each Object.entries(FILE_TYPE_GROUPS) as [key, group]}
									<button
										class="rounded border px-1.5 py-0.5 text-[10px] {rule.applyTo.includes(
											key as FileTypeKey
										)
											? 'bg-primary text-primary-foreground'
											: 'bg-background hover:bg-muted'}"
						...
						<!-- 删除按钮 -->
						<button
							class="text-muted-foreground hover:text-destructive mt-1 text-xs"
			...
			<p class="text-muted-foreground text-xs">翻译前去除匹配的内容。可按文件类型选择生效范围。</p>

			<!-- 正则测试 -->
			<div class="bg-muted/20 space-y-2 rounded border p-2">
				<div class="flex items-center gap-2">
					<input
						type="text"
						class="bg-background flex-1 rounded border px-2 py-1 text-xs"
		...
		<div class="space-y-2">
			...
			{#if testResult}
				<div
					class="flex items-center gap-2 rounded-md p-2 text-xs {testResult.success
			...
			<div class="flex gap-2">
				<Button variant="outline" size="sm" onclick={exportConfig} class="flex-1">导出配置</Button>
