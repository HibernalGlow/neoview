...
<div class="space-y-4">
	...
	<div class="grid grid-cols-2 gap-2">
		<div class="bg-muted/20 rounded-md border p-3 text-center">
			<div class="text-primary text-2xl font-bold">{formatNumber(stats.size)}</div>
			<div class="text-muted-foreground text-xs">缓存条目</div>
		</div>
		<div class="bg-muted/20 rounded-md border p-3 text-center">
			<div class="text-primary text-2xl font-bold">{formatCacheHitRate()}</div>
			<div class="text-muted-foreground text-xs">缓存命中率</div>
		</div>
	</div>
	...
	<div class="flex flex-wrap gap-2">
		...
		<Button
			...
			class="text-destructive hover:text-destructive flex-1"
		>
