...
<div class="space-y-3 text-xs">
	...
	<div class="flex items-center justify-between">
		<Label class="flex items-center gap-1.5 text-xs font-medium">
			{#if settings.enabled}
				<Eye class="text-primary h-3.5 w-3.5" />
			{:else}
				<EyeOff class="text-muted-foreground h-3.5 w-3.5" />
			{/if}
			翻译叠加层
		</Label>
		<Switch checked={settings.enabled} onCheckedChange={handleToggleEnabled} class="scale-90" />
	</div>
	<p class="text-muted-foreground -mt-1 text-[10px]">在图片上显示翻译区域和译文</p>

	<Separator />

	<!-- 显示选项 -->
	<div class="space-y-2">
		<p class="text-muted-foreground text-[10px] font-medium tracking-wide uppercase">显示选项</p>
	...
	<div class="space-y-2">
		<div class="flex items-center justify-between">
			...
			<span class="text-muted-foreground text-[10px]">{Math.round(settings.opacity * 100)}%</span>
		</div>
		<Slider
			type="single"
	...
	<!-- 统计信息 -->
	<div class="bg-muted/50 space-y-1 rounded-md p-2">
	...
	<!-- 操作按钮 -->
	<div class="space-y-2">
		<p class="text-muted-foreground text-[10px] font-medium tracking-wide uppercase">数据管理</p>
