...
<div class="space-y-2">
	<p class="text-muted-foreground text-[10px]">对比不同解码方法在各步骤的性能表现</p>
	...
	{#if detailedResults.length > 0}
		<div class="max-h-60 space-y-2 overflow-auto text-[10px]">
					...
					{#if result.success}
						<div class="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-0.5">
							...
							{#if result.original_dims}
								<div>
									原尺寸: <span class="font-mono"
										>{result.original_dims[0]}×{result.original_dims[1]}</span
									>
								</div>
							{/if}
							{#if result.output_dims}
								<div>
									输出: <span class="font-mono"
										>{result.output_dims[0]}×{result.output_dims[1]}</span
									>
								</div>
							{/if}
						</div>
					{:else if result.error}
						<div class="text-[9px] text-red-500">{result.error}</div>
					{/if}
				</div>
			{/each}
		</div>
