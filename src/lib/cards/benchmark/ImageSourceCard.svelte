...
<div class="space-y-3">
	...
	<div class="flex flex-wrap items-center gap-3 text-[10px]">
		<div class="flex items-center gap-1">
			...
			<div class="flex gap-1">
				{#each [3, 5, 10, 20] as count}
					<button
						type="button"
						class="rounded px-1.5 py-0.5 {testCount === count
		...
		<div class="flex items-center gap-1">
			...
			<div class="flex gap-1">
				{#each [1, 3, 5] as count}
					<button
						type="button"
						class="rounded px-1.5 py-0.5 {iterations === count
							? 'bg-primary text-primary-foreground'
							: 'bg-muted hover:bg-muted/80'}"
		...
		<label class="flex cursor-pointer items-center gap-1">
			<input type="checkbox" bind:checked={testCacheHit} class="h-3 w-3" />
			<span class="text-muted-foreground">缓存</span>
		</label>
		<Button onclick={resetCaches} variant="ghost" size="sm" class="h-5 px-1 text-xs">
	...
	{#if selectedArchive}
		<div class="text-muted-foreground text-[10px]">
			...
			<div class="mt-1 max-h-16 space-y-0.5 overflow-auto">
		...
		<div class="space-y-2">
			...
			{#each summary as s}
				<div class="space-y-1 rounded border p-2 text-[10px]">
					<div class="flex justify-between font-medium">
						...
						<span class="text-primary font-mono">{formatTime(s.avgTotal)}</span>
					</div>
					<div class="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-0.5">
						<div>
							加载: <span class="text-foreground font-mono">{formatTime(s.avgLoadData)}</span>
						</div>
						<div>
							Blob: <span class="text-foreground font-mono">{formatTime(s.avgBlobCreate)}</span>
						</div>
						<div>
							URL: <span class="text-foreground font-mono">{formatTime(s.avgUrlCreate)}</span>
						</div>
						<div>
							解码: <span class="text-foreground font-mono">{formatTime(s.avgImgDecode)}</span>
						</div>
					</div>
					<div class="text-muted-foreground flex justify-between border-t pt-1">
			...
			{#if summary.length === 2}
				...
				{#if blobSum && assetSum && blobSum.avgTotal > 0 && assetSum.avgTotal > 0}
					{@const ratio = blobSum.avgTotal / assetSum.avgTotal}
					<div class="rounded border p-2 text-center text-[10px]">
						{#if ratio > 1}
							<span class="font-medium text-green-500">ConvertFileSrc</span>
							<span class="text-muted-foreground"> 比 </span>
							<span class="font-medium text-blue-500">Blob</span>
							<span class="text-muted-foreground"> 快 </span>
							<span class="text-primary font-mono font-medium">{ratio.toFixed(2)}x</span>
						{:else}
							<span class="font-medium text-blue-500">Blob</span>
							<span class="text-muted-foreground"> 比 </span>
							<span class="font-medium text-green-500">ConvertFileSrc</span>
							<span class="text-muted-foreground"> 快 </span>
							<span class="text-primary font-mono font-medium">{(1 / ratio).toFixed(2)}x</span>
						{/if}
					</div>
				{/if}
			{/if}
				...
				{#each results as r}
					<div class="flex items-center justify-between rounded border px-2 py-1">
						<span class={r.mode === 'blob' ? 'text-blue-500' : 'text-green-500'}>
							{r.mode === 'blob' ? 'B' : 'A'}
							{#if r.cached}<span class="ml-1 text-yellow-500">C</span>{/if}
						</span>
						<span class="mx-2 flex-1 truncate">{r.fileName}</span>
