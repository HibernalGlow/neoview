	...
	<div class="flex flex-wrap items-center gap-2 text-[10px]">
		...
		<div class="flex gap-1">
			{#each [3, 5, 10, 20] as count}
				<button
					type="button"
					class="rounded px-2 py-0.5 {latencyTestCount === count
		...
		<label class="ml-2 flex cursor-pointer items-center gap-1">
			<input type="checkbox" bind:checked={includeWicLz4} class="h-3 w-3" />
		...
		<div class="space-y-2 rounded border p-2 text-[10px]">
			...
			{#if latencyStats.ipc}
				<div class="space-y-1">
					...
					<div class="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-0.5">
						<div>
							提取: <span class="text-foreground font-mono"
								>{latencyStats.ipc.avgExtract.toFixed(1)}ms</span
							>
						</div>
						<div>
							Blob: <span class="text-foreground font-mono"
								>{latencyStats.ipc.avgBlob.toFixed(1)}ms</span
							>
						</div>
						<div>
							URL: <span class="text-foreground font-mono"
								>{latencyStats.ipc.avgUrl.toFixed(2)}ms</span
							>
						</div>
						<div>
							解码: <span class="text-foreground font-mono"
								>{latencyStats.ipc.avgDecode.toFixed(1)}ms</span
							>
						</div>
						<div>
							渲染: <span class="text-foreground font-mono"
								>{latencyStats.ipc.avgRender.toFixed(2)}ms</span
							>
						</div>
						<div>
							总计: <span class="text-primary font-mono font-medium"
								>{latencyStats.ipc.avgTotal.toFixed(1)}ms</span
							>
						</div>
					</div>
				</div>
			...
			{#if latencyStats.temp}
				<div class="space-y-1 border-t pt-2">
					...
					<div class="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-0.5">
						<div>
							提取: <span class="text-foreground font-mono"
								>{latencyStats.temp.avgExtract.toFixed(1)}ms</span
							>
						</div>
						<div>
							URL: <span class="text-foreground font-mono"
								>{latencyStats.temp.avgUrl.toFixed(2)}ms</span
							>
						</div>
						<div>
							解码: <span class="text-foreground font-mono"
								>{latencyStats.temp.avgDecode.toFixed(1)}ms</span
							>
						</div>
						<div>
							渲染: <span class="text-foreground font-mono"
								>{latencyStats.temp.avgRender.toFixed(2)}ms</span
							>
						</div>
						<div class="col-span-2">
							总计: <span class="text-primary font-mono font-medium"
								>{latencyStats.temp.avgTotal.toFixed(1)}ms</span
							>
						</div>
					</div>
				</div>
			...
			{#if latencyStats.wicLz4}
				<div class="space-y-1 border-t pt-2">
					...
					<div class="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-0.5">
						<div>
							WIC解码: <span class="text-foreground font-mono"
								>{latencyStats.wicLz4.avgWicDecode.toFixed(1)}ms</span
							>
						</div>
						<div>
							LZ4压缩: <span class="text-foreground font-mono"
								>{latencyStats.wicLz4.avgLz4Compress.toFixed(1)}ms</span
							>
						</div>
						<div>
							LZ4解压: <span class="text-foreground font-mono"
								>{latencyStats.wicLz4.avgLz4Decompress.toFixed(1)}ms</span
							>
						</div>
						<div>
							渲染: <span class="text-foreground font-mono"
								>{latencyStats.wicLz4.avgRender.toFixed(1)}ms</span
							>
						</div>
						<div>
							压缩率: <span class="text-foreground font-mono"
								>{(latencyStats.wicLz4.avgCompressionRatio * 100).toFixed(0)}%</span
							>
						</div>
						<div>
							总计: <span class="text-primary font-mono font-medium"
								>{latencyStats.wicLz4.avgTotal.toFixed(1)}ms</span
							>
						</div>
					</div>
			...
			{#if latencyStats.speedup}
				<div class="border-t pt-2 text-center">
					...
					<span class="font-mono font-medium text-green-500">{latencyStats.speedup}x</span>
				</div>
			{/if}
		</div>
	{/if}
