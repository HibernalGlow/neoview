...
<div class="space-y-2">
	<div class="text-muted-foreground space-y-1 text-[10px]">
		<p>当前模式: <span class="text-primary font-mono">{currentRendererMode}</span></p>
		<p>
			ViewerJS: <span class="font-mono {viewerJSEnabled ? 'text-green-500' : 'text-red-500'}"
				>{viewerJSEnabled ? '启用' : '禁用'}</span
			>
		</p>
	</div>
	...
	<div class="flex items-center gap-2 text-[10px]">
		...
		<div class="flex gap-1">
			{#each [5, 10, 20, 50] as count}
				<button
					type="button"
					class="rounded px-2 py-0.5 {rendererTestCount === count
			...
			{#each rendererTestResults as result}
				<div class="space-y-1 rounded border p-2">
					<div class="flex justify-between font-medium">
						<span
							class:text-blue-500={result.mode === 'native'}
					...
					{#if result.success}
						<div class="text-muted-foreground">
							FPS: <span class="text-primary font-mono">{result.fps.toFixed(1)}</span>
						</div>
