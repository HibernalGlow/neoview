<script lang="ts">
	...
	} from '$lib/components/panels/folderPanel/stores/folderPanelStore.svelte';
	...
	import { activeTab } from '$lib/components/panels/folderPanel/stores/folderTabStore.svelte';
	...
	async function handleItemOpen(item: FsItem) {
		...
		if (item.isDir) {
			console.warn(
				'[FolderPanel] Unexpected folder in handleItemOpen, should be handled by FolderStack'
			);
			return;
		}
	...
	async function handleSearch(keyword: string) {
		...
		try {
			...
			if (hasTagFilter) {
				...
				const backendTags: [string, string, string][] = searchTags.map((t) => [
					t.cat,
					t.tag,
					t.prefix
				]);
			...
			if (favTags.length > 0 && resultsWithRating.length > 0) {
				try {
					...
					const countResults = await invoke<[string, number][]>(
						'batch_count_matching_collect_tags',
						{
							keys,
							collectTags,
							enableMixedGender: enableMixed
					...
					console.log(
						`[FolderPanel] 已附加收藏标签数，最大值: ${Math.max(...resultsWithRating.map((r) => r.collectTagCount || 0))}`
					);
				} catch (e) {
			...
			console.log(
				`[FolderPanel] 搜索完成，找到 ${resultsWithRating.length} 个结果`,
				resultsWithRating.slice(0, 5)
			);
	...
	// 处理搜索设置变更
	function handleSearchSettingsChange(settings: {
	...
	{#if $showSearchBar}
		<div class="relative">
			<div class="flex items-center gap-1">
				...
				<!-- 收藏标签快选按钮 -->
				<button
					class="hover:bg-accent shrink-0 rounded border p-1.5 {showFavoriteTagPanel
			...
			<FavoriteTagPanel
				...
				onUpdateEnableMixed={(v: boolean) => {
					mixedGenderStore.enabled = v;
				}}
			/>
		</div>
	{/if}
...
{#if renameDialogItem}
	<RenameDialog
		...
		title="重命名"
		initialValue={renameDialogItem.name}
