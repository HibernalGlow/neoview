<script lang="ts">
	...
	function handleToggleRandomTagBar() {
		...
		if (showRandomTagBar && randomTags.length === 0) tagActions.refreshRandomTags();
	}
	...
	$effect(() => {
		...
		const unsub1 = activeTabId.subscribe((v) => {
			currentActiveTabId = v;
		});
		const unsub2 = allTabs.subscribe((v) => {
			currentAllTabs = v;
		});
		return () => {
			unsub1();
...
<div class="bg-muted/10 flex h-full flex-col overflow-hidden">
	<BreadcrumbBar
		onNavigate={handleNavigate}
		{homePath}
		externalPath={isVirtualInstance ? propInitialPath : undefined}
	/>
	...
	<FolderToolbar
		...
		virtualMode={isVirtualInstance
			? propInitialPath?.includes('bookmark')
				? 'bookmark'
				: 'history'
			: null}
	/>
		...
		<div class="relative">
			<div class="flex items-center gap-1">
				...
				<button
					...
					title="收藏标签"
					><Star class="h-4 w-4 {showFavoriteTagPanel ? 'fill-primary' : ''}" /></button
				>
			</div>
			<FavoriteTagPanel
	...
	{#if showRandomTagBar}
		<div class="bg-muted/30 border-border flex items-center gap-1.5 border-b px-2 py-1.5">
			...
			<div class="flex flex-1 flex-wrap items-center gap-1">
				{#each randomTags as tag (tag.id)}<TagChip
						tag={tag.id}
						category={tag.cat}
						display={tag.display}
			...
			<button
				...
				title="刷新"><RefreshCw class="h-3.5 w-3.5" /></button
			>
		</div>
	{/if}
...
{#if renameDialogItem}<RenameDialog
