...
{#if bookInfo}
	<div class="space-y-2 text-sm">
		<div class="flex justify-between">
			...
			<span
				class={bookInfo.emmMetadata?.translatedTitle &&
				bookInfo.emmMetadata.translatedTitle !== bookInfo.name
					...
					: 'max-w-[200px] font-medium break-words'}
		...
		{#if bookInfo.emmMetadata?.translatedTitle && bookInfo.emmMetadata.translatedTitle !== bookInfo.name}
			<div class="flex justify-between">
				...
				<span class="max-w-[200px] font-mono text-xs break-words" title={bookInfo.name}>
					{bookInfo.name}
				</span>
			</div>
		{/if}
		<div class="flex justify-between">
			...
			<span class="max-w-[200px] font-mono text-xs break-words" title={bookInfo.path}>
				{bookInfo.path}
			</span>
		</div>
		<div class="flex justify-between">
...
{:else}
	<div class="text-muted-foreground py-2 text-center text-sm">暂无书籍信息</div>
{/if}
