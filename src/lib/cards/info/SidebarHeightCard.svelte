...
<div class="text-muted-foreground space-y-5 text-xs">
	<div class="flex items-center justify-between pb-1">
		...
		<div class="flex items-center gap-2">
			<Label for="show-drag-handle" class="cursor-pointer text-[10px]">显示拖拽手柄</Label>
			<Switch
				id="show-drag-handle"
	...
	<div class="bg-accent/10 border-border/30 space-y-2 rounded-lg border p-2.5">
		<div class="flex items-center justify-between gap-2">
			<Label for="blank-area-collapse" class="cursor-pointer text-[10px]"
				>空白区点击收回侧边栏</Label
			>
			<Switch
		...
		<p class="text-muted-foreground/70 text-[9px]">
	...
	<div class="grid grid-cols-2 gap-x-6 gap-y-4">
		<!-- 左侧边栏控制 -->
		<div class="space-y-3">
			<div class="mb-2 flex items-center gap-1.5 border-b border-blue-500/30 pb-1">
				<PanelLeft class="h-3.5 w-3.5 text-blue-500" />
				<span class="text-foreground text-[11px] font-bold">左侧边栏</span>
			</div>
			...
			<div class="space-y-1">
				<div class="flex items-center justify-between">
					...
					<span class="font-mono text-[10px] text-blue-400 tabular-nums"
						>{Math.round(leftCurrentHeight)}%</span
					>
				</div>
				<input
					type="range"
					...
					class="bg-muted h-1 w-full cursor-pointer appearance-none rounded-lg accent-blue-500"
				/>
			</div>
			...
			<div
				class="space-y-1"
				class:opacity-30={leftIsFullSize}
				class:pointer-events-none={leftIsFullSize}
			>
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-1 text-[10px]">
			...
			<div class="space-y-1">
				...
				<input
					...
					class="bg-muted h-1 w-full cursor-pointer appearance-none rounded-lg accent-blue-500"
				/>
				<div class="text-muted-foreground/40 flex justify-between px-0.5 text-[8px]">
					<span>贴边</span>
					<span>居中</span>
				</div>
			</div>
		...
		<!-- 右侧边栏控制 -->
		<div class="space-y-3">
			<div class="mb-2 flex items-center gap-1.5 border-b border-green-500/30 pb-1">
				<PanelRight class="h-3.5 w-3.5 text-green-500" />
				<span class="text-foreground text-[11px] font-bold">右侧边栏</span>
			</div>
			...
			<div class="space-y-1">
				<div class="flex items-center justify-between">
					...
					<span class="font-mono text-[10px] text-green-400 tabular-nums"
						>{Math.round(rightCurrentHeight)}%</span
					>
				</div>
				<input
					type="range"
					...
					class="bg-muted h-1 w-full cursor-pointer appearance-none rounded-lg accent-green-500"
				/>
			</div>
			...
			<div
				class="space-y-1"
				class:opacity-30={rightIsFullSize}
				class:pointer-events-none={rightIsFullSize}
			>
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-1 text-[10px]">
						<MoveVertical class="h-2.5 w-2.5" />
					...
					<span class="font-mono text-[10px] text-green-400 tabular-nums"
						>{$rightSidebarVerticalAlign}%</span
					>
				</div>
				<input
					type="range"
			...
			<div class="space-y-1">
				<div class="flex items-center justify-between">
					...
					<span class="font-mono text-[10px] text-green-400 tabular-nums"
						>{$rightSidebarHorizontalPos}%</span
					>
				</div>
				<input
					type="range"
	...
	<!-- 触发区域配置 -->
	<div class="bg-accent/10 border-border/30 space-y-3 rounded-lg border p-2.5">
		<div class="flex items-center gap-1.5">
			<MousePointer2 class="text-primary h-3 w-3" />
			<span class="text-foreground text-[10px] font-medium">触控区域 (px)</span>
		</div>
		...
		<div class="grid grid-cols-2 gap-x-4 gap-y-2">
			<div class="space-y-1">
				<div class="flex justify-between text-[9px]">
					...
					<span class="text-primary font-mono">{hoverAreas.leftTriggerWidth}</span>
				</div>
				<input
					type="range"
					min="4"
					...
					class="bg-muted accent-primary h-0.5 w-full cursor-pointer"
				/>
			</div>
			<div class="space-y-1">
				<div class="flex justify-between text-[9px]">
					<span class="text-muted-foreground/70">右边缘</span>
					<span class="text-primary font-mono">{hoverAreas.rightTriggerWidth}</span>
				</div>
				<input
					type="range"
					min="4"
			...
			<div class="space-y-1">
				<div class="flex justify-between text-[9px]">
					...
					<span class="text-primary font-mono">{hoverAreas.topTriggerHeight}</span>
				</div>
				<input
					type="range"
					min="2"
			...
			<div class="space-y-1">
				<div class="flex justify-between text-[9px]">
					...
					<span class="text-primary font-mono">{hoverAreas.bottomTriggerHeight}</span>
				</div>
				<input
					type="range"
					min="2"
