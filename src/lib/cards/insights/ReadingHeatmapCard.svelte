<script lang="ts">
	...
	import ReadingHeatmapChart, {
		type HeatmapCell
	} from '$lib/components/panels/insights/ReadingHeatmapChart.svelte';
...
<div class="space-y-3 overflow-hidden">
	{#if heatmap.topSlot && heatmap.topSlot.count > 0}
		<p class="text-muted-foreground text-xs">
			高峰时段: <span class="text-foreground font-medium"
				>{heatmap.topSlot.weekdayLabel} {heatmap.topSlot.hourLabel}</span
			>
		</p>
	{/if}
