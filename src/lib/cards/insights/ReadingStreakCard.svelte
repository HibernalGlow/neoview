<script lang="ts">
	...
	import ReadingStreakChart, {
		type StreakPoint
	} from '$lib/components/panels/insights/ReadingStreakChart.svelte';
...
<div class="space-y-3">
	<div class="grid grid-cols-3 gap-3 text-center">
		<div>
			<p class="text-muted-foreground text-xs">当前连续</p>
			<p class="text-lg font-semibold">{streak.currentStreak} 天</p>
		</div>
		<div>
			<p class="text-muted-foreground text-xs">最长连续</p>
			<p class="text-lg font-semibold">{streak.longestStreak} 天</p>
		</div>
		<div>
			<p class="text-muted-foreground text-xs">最近活跃</p>
