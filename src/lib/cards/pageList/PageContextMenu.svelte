<script lang="ts">
	...
	import { Copy, Trash2, FolderOpen, Play, ExternalLink } from '@lucide/svelte';
	...
	let { item, x, y, visible, onClose, onGoToPage, onDelete }: Props = $props();
	 ...
	 * 检查页面是否在压缩包内
	 */
	function getArchiveInfo(pageItem: PageItem): {
		isArchive: boolean;
		archivePath?: string;
		innerPath?: string;
	} {
	...
	async function handleCopy() {
		...
		try {
			...
			if (archiveInfo.isArchive && archiveInfo.archivePath && archiveInfo.innerPath) {
				...
				console.log(
					'[PageContextMenu] Extracting from archive:',
					archiveInfo.archivePath,
					archiveInfo.innerPath
				);
...
{#if visible && item}
	...
	>
		<div
			class=" text-popover-foreground min-w-40 overflow-hidden rounded-md border shadow-md backdrop-blur-md"
