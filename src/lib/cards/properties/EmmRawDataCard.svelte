...
<div class="space-y-2">
	{#if emmRawEntries.length === 0}
		<p class="text-muted-foreground py-4 text-center text-xs">暂无 EMM 数据库记录</p>
	{:else}
		<!-- 过滤输入 -->
		<div class="flex items-center gap-2">
			...
			/>
			<span class="text-muted-foreground text-[10px]">{emmRawEntries.length} 条</span>
		</div>
						...
						<Table.Row>
							{#each row.getVisibleCells() as cell (cell.id)}
								{#if cell.column.id === 'key'}
									...
									<Table.Cell
										class="text-muted-foreground max-w-[140px] truncate px-2 py-0.5 align-top"
									>
										<Tooltip.Root>
											<Tooltip.Trigger>
												<span class="inline-block w-full truncate text-left">{meta.label}</span>
											</Tooltip.Trigger>
											<Tooltip.Content>
								...
								{:else}
									{@const original = row.original}
									{@const meta = getFieldMeta(original.key)}
									<Table.Cell class="max-w-[200px] truncate px-2 py-0.5 text-right align-top">
										<Tooltip.Root>
											<Tooltip.Trigger>
											...
											<Tooltip.Content>
												<p class="max-w-xs text-[10px] break-all">{original.value}</p>
											</Tooltip.Content>
										</Tooltip.Root>
									</Table.Cell>
								{/if}
