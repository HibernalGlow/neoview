...
<div class="space-y-2 text-[11px]">
	{#if !bookInfo}
		<div class="text-muted-foreground py-4 text-center">
			<p>未打开书籍</p>
			<p class="mt-1 text-[10px]">打开书籍后显示标签</p>
		</div>
	{:else if !bookInfo.emmMetadata}
		<div class="text-muted-foreground py-4 text-center">
			<p>无 EMM 数据</p>
			<p class="mt-1 text-[10px]">此书籍没有关联的 EMM 元数据</p>
		</div>
	...
	{:else}
		<div class="flex flex-wrap gap-1">
			{#each allTags as tag}
				...
				<span
					class="border-border hover:bg-muted/50 inline-flex items-center rounded border px-1.5 py-0.5 text-[10px]"
		...
		<div class="text-muted-foreground pt-1 text-[10px]">
