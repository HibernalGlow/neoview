<script lang="ts">
	...
	function duplicateCondition(cond: UpscaleCondition) {
		const newCond = normalizeCondition(
			{ ...cond, id: '', name: `${cond.name} (副本)` },
			conditions.length
		);
		newCond.priority = conditions.length;
		persistConditions([...conditions, newCond]);
...
<div class="w-full space-y-3">
	{#if conditionalUpscaleEnabled}
			...
			<Tabs bind:value={activeTab} class="w-full">
				<TabsList class="flex h-auto flex-wrap gap-1">
					{#each conditions as cond (cond.id)}
						<TabsTrigger value={cond.id} class="flex h-7 items-center gap-1 px-2 py-1 text-xs">
							{cond.name}
							{#if !cond.enabled}<Badge variant="secondary" class="px-1 text-[8px]">禁用</Badge
								>{/if}
							{#if cond.action.skip}<Badge variant="destructive" class="px-1 text-[8px]">跳过</Badge
								>{/if}
						</TabsTrigger>
					{/each}
				</TabsList>
								...
								<div class="flex items-center justify-between">
									...
									<div class="flex items-center gap-1">
										...
										<Button
											...
											disabled={cond.priority === conditions.length - 1}
										>
										...
										<Button
											...
											size="sm"
											class="h-6 w-6 p-0"
											onclick={() => duplicateCondition(cond)}
										>
											<Copy class="h-3 w-3" />
										</Button>
										<Button
		...
		{:else}
			<Card class="p-4">
				<p class="text-muted-foreground text-center text-xs">暂无条件，点击添加</p>
			</Card>
		{/if}
	{:else}
		<Card class="p-4">
			<p class="text-muted-foreground text-center text-xs">启用条件超分后可配置规则</p>
		</Card>
	{/if}
</div>
...
<Dialog bind:open={importDialogOpen}>
	<DialogContent class="sm:max-w-xl">
		...
		<textarea
			class="bg-background h-48 w-full resize-none rounded-md border p-2 font-mono text-xs"
