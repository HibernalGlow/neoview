...
>
	...
	{#if !hideHeader}
		<div
			class="flex items-center justify-between {compact
				? 'px-2 py-1'
		...
		>
			...
			>
				{#if showDragHandle}
					<GripVertical class="text-muted-foreground h-3.5 w-3.5 cursor-grab" />
				{/if}
				...
				{#if !hideTitle}
					<span
						class="text-xs font-medium {orientation === 'horizontal'
			...
			<div
				class="group/buttons flex items-center gap-0.5 {orientation === 'horizontal'
					? 'mt-1 flex-col'
					: ''}"
			>
				<!-- 在新窗口打开按钮（悬停按钮区域显示） -->
				<button
					type="button"
					class="text-muted-foreground hover:bg-primary/20 hover:text-primary inline-flex h-5 w-5 items-center justify-center rounded opacity-0 transition-opacity group-hover/buttons:opacity-100"
				...
				{#if canHideActual}
					<button
						type="button"
						class="text-muted-foreground hover:bg-destructive/20 hover:text-destructive inline-flex h-5 w-5 items-center justify-center rounded opacity-0 transition-opacity group-hover/buttons:opacity-100"
						onclick={hideCard}
						title="隐藏卡片"
					>
				...
				{#if height && onHeightChange}
					<button
						type="button"
						class="text-muted-foreground hover:bg-muted hover:text-primary inline-flex h-5 w-5 items-center justify-center rounded"
						onclick={resetHeight}
						title="恢复默认高度"
					>
	...
	{#if isExpanded || hideHeader}
		<div
			...
			class="{hideHeader ? '' : compact ? 'px-2 pb-2' : 'px-3 pb-3'} {fullHeight
				? 'relative flex min-h-0 flex-1 flex-col overflow-hidden'
				: 'overflow-auto'} {orientation === 'horizontal' ? 'flex-1' : ''}"
		...
		{#if onHeightChange && !hideHeader}
			<button
				...
				class="{orientation === 'horizontal'
					...
					: 'h-2 cursor-ns-resize'} hover:bg-primary/20 group flex items-center justify-center"
			...
			>
				<div
					class="{orientation === 'horizontal'
						? 'h-8 w-0.5'
						: 'h-0.5 w-8'} bg-muted-foreground/30 group-hover:bg-primary/50 rounded-full"
				></div>
			</button>
		{/if}
	{/if}
