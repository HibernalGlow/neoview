...
import { settingsManager } from '$lib/settings/settingsManager';
	...
	type CardId = 'visibility' | 'viewerjs' | 'files' | 'detailed' | 'loadmode' | 'archives' | 'realworld' | 'results' | 'summary';
	...
	// ==================== 状态管理 ====================
	let cardOrder = $state<CardId[]>(['visibility', 'viewerjs', 'files', 'detailed', 'loadmode', 'archives', 'realworld', 'results', 'summary']);
				...
				{#if showCards.viewerjs}
					<div class="space-y-2">
						...
						<div class="text-[10px] text-muted-foreground">
							渲染模式: <span class="font-mono text-cyan-500">{settings.view.renderer?.mode ?? 'stack'}</span>
						</div>
					</div>
				{/if}
			</div>
