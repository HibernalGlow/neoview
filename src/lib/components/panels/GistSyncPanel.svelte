<script lang="ts">
	...
	import {
		settingsManager as exportManager,
		type FullExportPayload
	} from '$lib/stores/settingsManager';
	import {
		Github,
		LogIn,
		LogOut,
	...
	function openGitHubTokenPage() {
		window.open(
			'https://github.com/settings/tokens/new?scopes=gist&description=NeoView%20Settings%20Sync',
			'_blank'
		);
	}
...
<div class="space-y-4 border-t pt-4">
	...
	<p class="text-muted-foreground text-xs">将设置备份到 GitHub Gist，支持跨设备同步</p>
	...
	{#if isLoggedIn && user}
		<div class="flex items-center gap-3 rounded-lg border bg-green-500/10 p-3">
			...
			<div class="flex-1">
				...
				<p class="text-muted-foreground text-xs">@{user.login}</p>
			</div>
		...
		<div class="space-y-3">
			<div class="flex items-center justify-between">
				...
				{#if config.gistId}
					<div class="flex items-center gap-2">
						<code class="bg-muted rounded px-2 py-1 text-xs">{config.gistId.slice(0, 8)}...</code>
						<Button
							variant="ghost"
							size="icon"
							class="h-7 w-7"
							onclick={() => copyToClipboard(config.gistId, 'gistId')}
						>
							{#if copiedGistId}
				...
				{:else}
					<span class="text-muted-foreground text-xs">未关联</span>
				{/if}
			</div>
		...
		<div class="space-y-3 border-t pt-4">
			<div class="flex items-center justify-between">
				...
				<Button
					...
					size="sm"
					onclick={() => {
						showGistList = !showGistList;
						if (showGistList) loadUserGists();
					}}
				>
			...
			{#if showGistList}
				<div class="max-h-48 space-y-2 overflow-auto rounded-lg border p-2">
					...
					{:else if userGists.length === 0}
						<p class="text-muted-foreground py-4 text-center text-xs">没有找到 Gist</p>
					{:else}
						{#each userGists as gist}
							<button
								class="hover:bg-accent flex w-full items-center gap-2 rounded-md p-2 text-left {config.gistId ===
								gist.id
									? 'bg-primary/10'
									: ''}"
							...
							>
								<div class="flex-1 truncate">
									...
									<p class="text-muted-foreground text-xs">{Object.keys(gist.files).join(', ')}</p>
								</div>
			...
			<div class="flex gap-2">
				<Input bind:value={gistIdInput} placeholder="或输入 Gist ID..." class="flex-1" />
				<Button
					variant="outline"
					size="icon"
					onclick={handleUseGistId}
		...
		<div class="space-y-4 border-t pt-4">
			...
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-muted-foreground text-xs">定时自动上传设置到 Gist</p>
				</div>
			...
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-muted-foreground text-xs">设置为公开后任何人都可以查看</p>
				</div>
		...
		<div class="space-y-4">
			<div class="bg-muted/50 rounded-lg border p-4">
				...
				<ol class="text-muted-foreground list-inside list-decimal space-y-1 text-sm">
					...
					<li>登录 GitHub 账号</li>
					<li>确保勾选 <code class="bg-muted rounded px-1">gist</code> 权限</li>
	...
	{#if statusMessage}
		<div
			class="flex items-center gap-2 text-sm {status === 'success'
				? 'text-green-500'
				: status === 'error'
					? 'text-red-500'
