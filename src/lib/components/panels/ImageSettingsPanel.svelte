	...
	<Tabs.Root bind:value={activeTab} class="w-full">
		...
		<Tabs.Content value="image" class="mt-4 space-y-4">
			...
			<div class="space-y-2">
				...
				<p class="text-muted-foreground text-xs">控制 NeoView 默认识别为图片的文件扩展名。</p>
			</div>
			...
			<div class="space-y-3">
					...
					<div class="flex flex-col gap-0.5">
						...
						<div class="mt-1 flex items-center gap-2">
							...
							{:else if testStatus === 'unsupported'}
								<span class="text-destructive flex items-center gap-1 text-[10px]">
				...
				<p class="text-muted-foreground text-xs">
					...
					<span class="font-medium text-yellow-500"
						>⚠ 需要 WebView2 Runtime ≥ 145 / 重启后生效</span
					>
				</p>
			</div>
		</Tabs.Content>
				...
				<div class="space-y-2">
					<div class="flex items-center justify-between gap-2">
						...
						<input
							...
							class="bg-background w-24 rounded border px-2 py-1 text-right text-sm"
							onchange={(event) => {
								const target = event.target as HTMLInputElement;
