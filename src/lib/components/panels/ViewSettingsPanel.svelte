	...
	<Tabs.Root bind:value={activeTab} class="w-full">
		...
		<Tabs.Content value="display" class="mt-4 space-y-4">
			...
			<div class="space-y-2">
				...
				<NativeSelect class="w-full max-w-xs" bind:value={currentSettings.view.defaultZoomMode}>
			...
			<div class="space-y-2">
				...
				{#if currentSettings.view.backgroundMode === 'ambient'}
					<p class="text-muted-foreground text-xs">
						✨
						从图片中提取主色调，生成类似苹果灵动岛的流动渐变效果。详细设置请使用「流光溢彩」卡片。
					</p>
				{/if}
			</div>
		</Tabs.Content>
									...
									<Input
										...
										oninput={(e) => {
											...
											if (!Number.isNaN(val)) {
												...
												settingsManager.updateNestedSettings('view', {
													mouseCursor: {
														...latest.view.mouseCursor,
														hideDelay: Math.max(0.5, Math.min(val, 5.0))
													}
												});
											}
										}}
							...
							<Slider
								...
								onValueChange={(v: number | number[]) => {
									...
									settingsManager.updateNestedSettings('view', {
										mouseCursor: {
											...latest.view.mouseCursor,
											hideDelay: Array.isArray(v) ? v[0] : v
										}
									});
								}}
								class="w-full max-w-xs"
										...
										oninput={(e) => {
											...
											if (!Number.isNaN(val)) {
												...
												settingsManager.updateNestedSettings('view', {
													mouseCursor: {
														...latest.view.mouseCursor,
														showMovementThreshold: Math.max(5, Math.min(val, 100))
													}
												});
											}
										}}
							...
							<Slider
								...
								onValueChange={(v: number | number[]) => {
									...
									settingsManager.updateNestedSettings('view', {
										mouseCursor: {
											...latest.view.mouseCursor,
											showMovementThreshold: Array.isArray(v) ? v[0] : v
										}
									});
								}}
								class="w-full max-w-xs"
