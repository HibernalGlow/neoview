		...
		<div class="space-y-2">
			...
			{#if currentSettings.view.backgroundMode === 'ambient'}
				<div class="space-y-3 rounded-lg border p-3 bg-muted/30">
					<p class="text-xs text-muted-foreground">✨ 从图片中提取多个主色调，生成类似苹果灵动岛的流动渐变效果</p>
					<!-- 样式选择 -->
					<div class="flex items-center justify-between gap-2">
						<Label class="text-sm">动画样式</Label>
						<NativeSelect
							class="w-32"
							value={currentSettings.view.ambient?.style ?? 'vibrant'}
							onchange={(e) =>
								settingsManager.updateNestedSettings('view', {
									ambient: {
										...currentSettings.view.ambient,
										speed: currentSettings.view.ambient?.speed ?? 8,
										blur: currentSettings.view.ambient?.blur ?? 80,
										opacity: currentSettings.view.ambient?.opacity ?? 0.8,
										style: (e.currentTarget as HTMLSelectElement).value as 'gentle' | 'vibrant' | 'dynamic'
									}
								})}
						>
							<NativeSelectOption value="gentle">柔和</NativeSelectOption>
							<NativeSelectOption value="vibrant">鲜艳</NativeSelectOption>
							<NativeSelectOption value="dynamic">动感</NativeSelectOption>
						</NativeSelect>
					</div>
					<!-- 速度 -->
					<div class="flex items-center justify-between gap-2">
						<Label class="text-sm">动画速度</Label>
						<div class="flex items-center gap-2">
							<input
								type="range"
								min="2"
								...
								step="1"
								class="w-24"
								value={currentSettings.view.ambient?.speed ?? 8}
								onchange={(e) =>
									settingsManager.updateNestedSettings('view', {
										ambient: {
											...currentSettings.view.ambient,
											...
											blur: currentSettings.view.ambient?.blur ?? 80,
											opacity: currentSettings.view.ambient?.opacity ?? 0.8,
											style: currentSettings.view.ambient?.style ?? 'vibrant'
										}
									})}
							/>
							<span class="text-xs text-muted-foreground w-8">{currentSettings.view.ambient?.speed ?? 8}s</span>
						</div>
					</div>
					<!-- 模糊程度 -->
					<div class="flex items-center justify-between gap-2">
						<Label class="text-sm">模糊程度</Label>
						<div class="flex items-center gap-2">
							<input
								type="range"
								min="20"
								...
								step="10"
								class="w-24"
								value={currentSettings.view.ambient?.blur ?? 80}
								onchange={(e) =>
									settingsManager.updateNestedSettings('view', {
										ambient: {
											...currentSettings.view.ambient,
											speed: currentSettings.view.ambient?.speed ?? 8,
											blur: Number((e.currentTarget as HTMLInputElement).value),
											opacity: currentSettings.view.ambient?.opacity ?? 0.8,
											style: currentSettings.view.ambient?.style ?? 'vibrant'
										}
									})}
							/>
							<span class="text-xs text-muted-foreground w-10">{currentSettings.view.ambient?.blur ?? 80}px</span>
						</div>
					</div>
					<!-- 透明度 -->
					<div class="flex items-center justify-between gap-2">
						<Label class="text-sm">效果强度</Label>
						<div class="flex items-center gap-2">
							<input
								type="range"
								min="0.3"
								...
								step="0.1"
								class="w-24"
								value={currentSettings.view.ambient?.opacity ?? 0.8}
								onchange={(e) =>
									settingsManager.updateNestedSettings('view', {
										ambient: {
											...currentSettings.view.ambient,
							...
							/>
							<span class="text-xs text-muted-foreground w-10">{Math.round((currentSettings.view.ambient?.opacity ?? 0.8) * 100)}%</span>
						</div>
					</div>
				</div>
			{/if}
