<script lang="ts">
	 ...
	 * 负责显示收藏标签和手动标签的快速选择
	 */
	import { Heart, Tag, ChevronUp, ChevronDown, RefreshCcw } from '@lucide/svelte';
	import * as Button from '$lib/components/ui/button';
	import { emmMetadataStore, emmTranslationStore } from '$lib/stores/emmMetadata.svelte';
	import {
		favoriteTagStore,
		categoryColors,
		mixedGenderStore,
	...
	// 状态
	let showFavoriteTagsCard = $state(true);
...
<div class="bg-muted/30 rounded-lg border">
	<button
		...
		onclick={() => (showFavoriteTagsCard = !showFavoriteTagsCard)}
	>
		<div class="flex items-center gap-1.5">
			<Heart class="h-3.5 w-3.5 text-pink-500" />
			<span>收藏标签快选</span>
			<span class="text-muted-foreground">({favoriteTagStore.tags.length})</span>
		</div>
	...
	{#if showFavoriteTagsCard}
		<div class="space-y-2 px-2 pb-2 text-[11px]">
			<!-- 手动标签区域（全局唯一标签，用于搜索） -->
			<div class="border-border/50 border-b pb-2">
				<div class="mb-1 flex items-center justify-between">
					<button
						type="button"
						class="hover:text-primary flex items-center gap-1 font-medium"
					...
					<span class="text-muted-foreground text-[10px]">{allManualTags.length} 种</span>
				</div>

				{#if allManualTags.length === 0}
					<p class="text-muted-foreground py-1 text-center text-[10px]">暂无手动标签</p>
					<p class="text-muted-foreground/70 text-center text-[9px]">右键文件 → 编辑标签</p>
				{:else}
					<div class="flex flex-wrap gap-1">
						{#each allManualTags as mt}
							...
							<span
								class="inline-flex items-center gap-0.5 rounded border-2 border-dashed px-1.5 py-0.5 text-[10px]"
								style="border-color: {color}; background: color-mix(in srgb, {color} 12%, transparent); color: {color};"
								title="{NAMESPACE_LABELS[mt.namespace]}: {mt.tag}{translated !== mt.tag
									? ` (${translated})`
									: ''} ({mt.count}个文件)"
							>
								<span class="opacity-70">{mt.namespace.slice(0, 1)}:</span>
								<span>{translated}</span>
								<span class="text-[9px] opacity-50">×{mt.count}</span>
							</span>
						{/each}
					</div>
				{/if}
			...
			<!-- 控制栏 -->
			<div class="border-border/50 flex items-center justify-between gap-2 border-b py-1">
				...
				<Button.Root
					variant="ghost"
					size="sm"
					class="h-6 gap-1 px-2 text-[10px] {isReloadingFavoriteTags ? 'animate-spin' : ''}"
			...
			{#if favoriteTagGroups.length === 0}
				<div class="text-muted-foreground py-4 text-center">
					<p>暂无收藏标签</p>
					<p class="mt-1 text-[10px]">点击刷新从 EMM 导入</p>
				</div>
			{:else}
				<div class="max-h-[300px] space-y-2 overflow-y-auto">
					{#each favoriteTagGroups as group}
						<div>
							<div class="bg-muted/50 mb-1 flex items-center justify-between rounded px-1 py-0.5">
								<span class="font-semibold" style="color: {categoryColors[group.name] || '#666'}">
									{group.name}
								</span>
								<span class="text-muted-foreground text-[10px]">{group.tags.length}</span>
							</div>
							<div class="flex flex-wrap gap-1">
								{#each group.tags as tag (tag.id)}
									...
									<span
										class="inline-flex items-center gap-1 rounded border px-1.5 py-0.5 text-[10px] {isVariant
									...
									>
										<span
											class="h-1.5 w-1.5 {isVariant ? 'rounded-sm' : 'rounded-full'}"
			...
			<p class="text-muted-foreground border-border/50 border-t pt-1 text-[10px]">
				在文件夹面板搜索栏点击「★ 标签」使用快选
			</p>
		</div>
	{/if}
