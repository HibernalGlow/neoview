<script lang="ts">
	...
	const groupedTags = $derived(() => {
		...
		const groupMap = new Map<
			string,
			{ category: string; shortCategory: string; items: typeof tags }
		>();
	...
	$effect(() => {
		...
		try {
			const storedView = localStorage.getItem(TAG_VIEW_MODE_STORAGE_KEY) as
				| 'flat'
				| 'grouped'
				| null;
			...
			const storedFilter = localStorage.getItem(TAG_FILTER_MODE_STORAGE_KEY) as
				| 'all'
				| 'collect'
				| null;
	...
	>
		...
		<div class="flex items-center justify-between gap-2">
			<div class="flex items-center gap-2 text-sm font-semibold">
				<Tag class="h-4 w-4" />
				<span>标签</span>
				<span class="text-muted-foreground ml-2 text-[10px] font-normal opacity-50">
					(已加载收藏: {collectTags.length})
				</span>

				<!-- 视图切换按钮 -->
				<div class="ml-auto flex items-center gap-1 text-[10px] font-normal">
			...
			{#if tagViewMode === 'flat'}
				...
				<div class="flex flex-wrap gap-1.5">
					{#each displayTags() as tagInfo}
						<span
							class="inline-flex items-center rounded border px-2 py-1 text-xs {tagInfo.isCollect
								? 'font-semibold'
