<script lang="ts">
	...
	let { visible, onClose, onAppendTag }: Props = $props();
...
{#if visible}
	<div
		class="favorite-tag-panel bg-popover/80 absolute top-full right-0 left-0 z-50 mt-1 rounded-lg border shadow-lg backdrop-blur-md"
	...
	>
		...
		<div class="flex items-center justify-between gap-2 border-b px-3 py-2">
			...
			<div class="flex items-center gap-2">
				...
				<!-- 关闭按钮 -->
				<button class="hover:bg-accent rounded p-1" onclick={onClose}>
					<X class="h-3.5 w-3.5" />
				</button>
			</div>
		</div>
		...
		<div class="flex-1 overflow-y-auto p-3" style="height: calc(100% - 44px);">
			<FavoriteTagsCard interactive={true} onTagClick={handleTagClick} />
		</div>
		...
		<div
			class="hover:bg-primary/20 absolute right-0 bottom-0 left-0 h-1.5 cursor-ns-resize"
