<script lang="ts">
	...
	let globalFolderTreeConfigValue = $state<FolderTreeConfig>({
		visible: false,
		layout: 'left',
		size: 200
	});
	...
	// ==================== 派生状态 ====================
	let folderTreeConfig = $derived<FolderTreeConfig>(
		virtualMode
			? virtualMode === 'history'
	...
	let viewStyle = $derived<FolderViewStyle>(
		virtualMode
			? virtualMode === 'history'
				? virtualPanelSettingsStore.historyViewStyle
				: virtualPanelSettingsStore.bookmarkViewStyle
			: globalViewStyleValue
	);

	let multiSelectMode = $derived(
		virtualMode
			? virtualMode === 'history'
	...
	let sortConfig = $derived<SortConfig>(
		virtualMode
			? {
					field:
						virtualMode === 'history'
							? virtualPanelSettingsStore.historySortField
							: virtualPanelSettingsStore.bookmarkSortField,
					order:
						virtualMode === 'history'
							? virtualPanelSettingsStore.historySortOrder
							: virtualPanelSettingsStore.bookmarkSortOrder
				}
			: globalSortConfigValue
	);

	let itemTypeFilter = $derived<VirtualItemTypeFilter>(
		virtualMode
			? virtualMode === 'history'
	...
	let penetrateMode = $derived(
		virtualMode
			? virtualMode === 'history'
				? virtualPanelSettingsStore.historyPenetrateMode
				: virtualPanelSettingsStore.bookmarkPenetrateMode
			: globalPenetrateModeValue
	);

	let inlineTreeMode = $derived(
		virtualMode
			? virtualMode === 'history'
	...
	let showToolbarTooltip = $derived(
		virtualMode
			? virtualMode === 'history'
				? virtualPanelSettingsStore.historyShowToolbarTooltip
				: virtualPanelSettingsStore.bookmarkShowToolbarTooltip
			: false
	);

	let thumbnailWidthPercent = $derived(
		virtualMode
			? virtualMode === 'history'
				? virtualPanelSettingsStore.historyThumbnailWidthPercent
				: virtualPanelSettingsStore.bookmarkThumbnailWidthPercent
	...
	function toggleSortPanel() {
		const was = sortPanelExpanded;
		closePanels();
		sortPanelExpanded = !was;
		if (!was) refreshSortLockSettings();
	}
	function toggleViewPanel() {
		const was = viewPanelExpanded;
		closePanels();
		viewPanelExpanded = !was;
	}
	function toggleTreePanel() {
		const was = treePanelExpanded;
		closePanels();
		treePanelExpanded = !was;
	}
	function toggleTypeFilterBar() {
		const was = typeFilterBarExpanded;
		closePanels();
		typeFilterBarExpanded = !was;
	}
	...
	function handleSetDefaultSortScope(scope: 'global' | 'tab') {
		if (virtualMode) return;
		folderTabActions.setDefaultSortScope(scope);
		refreshSortLockSettings();
	}
	function handleSetCurrentSortAsDefault(scope?: 'global' | 'tab') {
		if (virtualMode) return;
		folderTabActions.setCurrentSortAsDefault(scope);
		refreshSortLockSettings();
	}
	function handleClearFolderSortMemory(path?: string) {
		if (virtualMode) return;
		folderTabActions.clearFolderSortMemory(path);
		refreshSortLockSettings();
	}
	...
	function handleToggleShowPenetrateSettingsBar() {
...
<div
	class={vertical
		? 'flex h-full flex-col items-center gap-1 overflow-y-auto px-1 py-2'
...
{#if typeFilterBarExpanded && virtualMode}
	<TypeFilterBar {itemTypeFilter} onSetItemTypeFilter={handleSetItemTypeFilter} />
{/if}
