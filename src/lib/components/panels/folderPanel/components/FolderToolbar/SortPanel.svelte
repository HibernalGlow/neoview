...
<div
	class="border-border/50 flex items-center gap-1 overflow-x-auto border-t px-2 py-1 whitespace-nowrap"
>
	...
	<div class="bg-muted/60 inline-flex items-center gap-0.5 rounded-full p-0.5 shadow-inner">
		{#each sortFields as field}
			...
			<Tooltip.Root>
				...
				<Tooltip.Content>
					<p>
						{field.label}{sortConfig.field === field.value
	...
	<Tooltip.Root>
		<Tooltip.Trigger>
			<Button variant="ghost" size="icon" class="h-6 w-6" onclick={onToggleSortOrder}>
		...
		<Tooltip.Root>
			<Tooltip.Trigger>
				<Button
					...
					variant="ghost"
					class="h-6 w-6"
					onclick={() => (showSortRuleDialog = true)}
				>
	...
	<Dialog.Root bind:open={showSortRuleDialog}>
		<Dialog.Content class="sm:max-w-130">
			<Dialog.Header>
				...
				<Dialog.Description
					>优先级：临时规则（当前文件夹） &gt; 文件夹记忆 &gt; 标签默认/全局默认</Dialog.Description
				>
			</Dialog.Header>
			...
			<div class="space-y-3">
				<div class="bg-muted/60 inline-flex items-center gap-0.5 rounded-full p-0.5 shadow-inner">
					...
					>
						<PanelLeftOpen class="mr-1 h-3 w-3" />标签默认
					</Button>
					<Button
					...
					>
						<Globe class="mr-1 h-3 w-3" />全局默认
					</Button>
				</div>

				<div class="flex flex-wrap gap-2">
					<Button size="sm" variant="ghost" class="h-7" onclick={() => onSetCurrentSortAsDefault()}>
						<Gauge class="mr-1 h-3 w-3" />将当前排序设为默认
					</Button>
					<Button
						size="sm"
						variant="ghost"
						class="h-7"
						onclick={() => onSetCurrentSortAsDefault('global')}
					>
						<Globe class="mr-1 h-3 w-3" />设为全局默认
					</Button>
					<Button size="sm" variant="ghost" class="h-7" onclick={() => onClearFolderSortMemory()}>
						<Database class="mr-1 h-3 w-3" />清空文件夹记忆
					</Button>
				</div>

				<div class="border-border/50 border-t pt-2 text-xs">
					<Table.Root>
						<Table.Header>
							<Table.Row>
						...
						<Table.Body>
							<Table.Row>
								...
								<Table.Cell
									><Badge variant={sortLockSettings.hasTemporaryRule ? 'default' : 'secondary'}
										>{sortLockSettings.hasTemporaryRule ? '启用' : '未启用'}</Badge
									></Table.Cell
								>
							</Table.Row>
							<Table.Row>
								...
								<Table.Cell
									><Badge
										variant={sortLockSettings.sortSource === 'memory' ? 'default' : 'secondary'}
										>{sortLockSettings.sortSource === 'memory' ? '当前命中' : '按路径命中'}</Badge
									></Table.Cell
								>
							</Table.Row>
							<Table.Row>
								...
								<Table.Cell
									>{sortLockSettings.defaultScope === 'tab' ? '标签默认' : '全局默认'}</Table.Cell
								>
								<Table.Cell
									><Badge
										variant={sortLockSettings.sortSource === 'tab-default' ||
										sortLockSettings.sortSource === 'global-default'
											? 'default'
											: 'secondary'}>{sortLockSettings.sortSource}</Badge
									></Table.Cell
								>
							</Table.Row>
						</Table.Body>
