...
<div class="flex flex-wrap items-center gap-4 text-xs">
	...
	{#if virtualMode}
		<div class="flex items-center gap-2">
			...
			<span class="text-muted-foreground/60 text-[10px]"> 鼠标悬停时显示按钮提示 </span>
		</div>
	{/if}
	...
	{#if virtualMode}
		<div class="flex items-center gap-2">
			<FolderSync class="text-muted-foreground h-3.5 w-3.5" />
	...
	<!-- 默认评分 -->
	<div class="flex items-center gap-2">
		<Star class="text-muted-foreground h-3.5 w-3.5" />
		...
		<input
			...
			class="bg-background h-6 w-14 rounded border px-2 text-center text-xs"
		/>
	...
	<!-- 双击空白处行为 -->
	<div class="flex items-center gap-2">
		<MousePointerClick class="text-muted-foreground h-3.5 w-3.5" />
	...
	<div class="flex items-center gap-2">
		...
		<span class="text-muted-foreground/60 text-[10px]"> 列表底部显示返回按钮 </span>
	</div>
</div>
