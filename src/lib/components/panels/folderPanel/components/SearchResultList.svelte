<script lang="ts">
	...
	const searchResults = $derived.by(() =>
		externalSearchResults ? get(externalSearchResults) : $tabSearchResults
	);
	const isSearching = $derived.by(() =>
		externalIsSearching ? get(externalIsSearching) : $tabIsSearching
	);
	const searchKeyword = $derived.by(() =>
		externalSearchKeyword ? get(externalSearchKeyword) : $tabSearchKeyword
	);
	...
	let sortedResults = $derived.by(() => {
		...
		if (field === 'size') {
			const foldersNeedingSize = results.filter(
				(item) =>
...
<div class="search-result-list flex h-full">
	<!-- 主内容区 -->
	<div bind:this={scrollContainer} class="flex-1 overflow-auto" onscroll={handleScroll}>
		{#if isSearching}
		...
		{:else if searchResults.length === 0 && searchKeyword}
			<div class="flex flex-col items-center justify-center py-12 text-center">
				...
				<p class="text-muted-foreground/70 mt-1 text-sm">搜索词: "{searchKeyword}"</p>
			</div>
		{:else if sortedResults.length > 0}
			<div class="p-2">
				...
				<div
					...
					style={viewMode === 'thumbnail'
						? `grid-template-columns: repeat(auto-fill, minmax(${$thumbnailWidthPercent}%, 1fr))`
						: ''}
				>
					{#each sortedResults as item (item.path)}
						<div class="search-result-item">
							...
							<!-- 搜索结果显示完整路径 -->
							<div class="px-1 pb-1">
								<p class="text-muted-foreground/70 truncate text-[10px]" title={item.path}>
									{item.path}
								</p>
							</div>
						</div>
		...
		{:else}
			<div class="flex flex-col items-center justify-center py-12 text-center">
				<FolderOpen class="text-muted-foreground/50 mb-4 h-12 w-12" />
