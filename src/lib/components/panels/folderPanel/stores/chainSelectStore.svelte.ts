...
const CHAIN_SELECT_DEBUG = false;

function debugChainSelect(...args: unknown[]): void {
	if (CHAIN_SELECT_DEBUG) {
		console.debug(...args);
	}
}
...
export function toggleChainSelectMode(tabId: string) {
	debugChainSelect('[ChainSelectStore] toggleChainSelectMode called, tabId:', tabId);
	chainSelectModeByTab.update((modes) => {
		const newEnabled = !modes[tabId];
		debugChainSelect('[ChainSelectStore] 当前状态:', modes[tabId], '-> 新状态:', newEnabled);
		// 关闭链选模式时清除锚点
		if (!newEnabled) {
