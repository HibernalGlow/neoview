...
import {
	saveSharedSortSettings,
	resolveSortForPath,
	normalizeSortPathKey,
	isVirtualPath
} from './utils.svelte';
...
function resolveForCurrentPath(tab: FolderTabState): {
...
export function setSort(field: FolderSortField, order?: FolderSortOrder): void {
	updateActiveTab((tab) => {
		...
		const hasTempRuleForCurrentPath =
			...
			normalizeSortPathKey(tab.temporarySortRule.path) === normalizedCurrentPath;
...
/** 解析指定路径应该使用的排序（导航时调用） */
export function resolveSortForTab(
	tab: FolderTabState,
	path: string
): {
	sortField: FolderSortField;
