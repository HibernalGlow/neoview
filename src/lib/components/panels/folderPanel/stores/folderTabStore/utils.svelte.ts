...
import { STORAGE_KEYS, VIRTUAL_PATHS, DEFAULT_PANEL_LAYOUT_SETTINGS } from './types';
...
export function saveTabsState(state: FolderTabsState): void {
	try {
		const realTabs = state.tabs.filter(
			(tab) => !isVirtualPath(tab.currentPath) && !isVirtualPath(tab.homePath)
		);
