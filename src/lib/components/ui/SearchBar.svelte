<script lang="ts">
	...
	function removeTag(tagToRemove: { cat: string; tag: string; prefix: string }) {
		...
		const patterns = [
			new RegExp(
				`\\s*${tagToRemove.prefix}[a-z]+:"${tagToRemove.tag.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"\\$`,
				'g'
			)
		];
			...
			if (saved) {
				...
				if (Array.isArray(parsed) && parsed.length > 0) {
					if (typeof parsed[0] === 'string') {
						// 旧版本：字符串数组
						searchHistory = parsed.map((query: string) => ({
							query,
...
<div class="search-bar-container border-border flex items-center gap-2 border-b px-2 py-2">
	<div class="relative flex-1">
		<!-- 搜索输入框 -->
		<div class="flex gap-1">
			<div
				class="border-border focus-within:ring-ring relative flex min-h-[36px] flex-1 flex-wrap items-center gap-1 rounded-md border bg-transparent px-2 py-1 focus-within:ring-2 focus-within:ring-offset-2"
			>
				...
				{#each parsedTags as tag}
					...
					<span
						class="inline-flex shrink-0 items-center gap-1 rounded border px-1.5 py-0.5 text-[11px] {tag.prefix ===
						'-'
				...
				{#if storageKey}
					<button
						class="hover:bg-accent shrink-0 rounded p-1 disabled:opacity-50"
						onclick={() => {
		...
		{#if showHistory && searchHistory.length > 0}
			<div
				class="search-history bg-popover/80 border-border absolute top-full right-0 left-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-lg border shadow-lg backdrop-blur-md"
			>
				{#each searchHistory as item (item.query)}
					<div
						class="hover:bg-accent group flex w-full cursor-pointer items-center justify-between px-3 py-2 text-left text-sm"
						role="button"
						tabindex="0"
					...
					>
						<div class="flex min-w-0 flex-1 items-center gap-2">
							<Search class="text-muted-foreground h-4 w-4 shrink-0" />
							<span class="text-foreground truncate">{item.query}</span>
						</div>
						<div class="flex items-center gap-2">
							<span class="text-muted-foreground text-xs"
								>{formatSearchHistoryTime(item.timestamp)}</span
							>
							<button
								class="hover:bg-accent-foreground/10 shrink-0 rounded p-1 opacity-0 group-hover:opacity-100"
								onclick={(e) => {
									e.stopPropagation();
				...
				<div class="border-border border-t p-2">
					<button
						class="text-muted-foreground hover:bg-accent w-full rounded px-3 py-1 text-xs"
		...
		{#if showSettings && searchSettings && onSettingsChange}
			<div
				class="search-settings bg-popover/80 border-border absolute top-full right-0 z-50 mt-1 min-w-[200px] rounded-lg border p-2 shadow-lg backdrop-blur-md"
			>
				<div class="space-y-3">
					<div class="pb-2">
						<h4 class="text-foreground mb-2 text-xs font-semibold">搜索选项</h4>
						...
						{#if searchSettings.includeSubfolders !== undefined}
							<label class="text-foreground flex items-center gap-2 text-sm">
								<input
									...
									class="border-border text-primary focus:ring-primary accent-primary rounded"
								/>
								<span>搜索子文件夹</span>
							</label>
						{/if}
