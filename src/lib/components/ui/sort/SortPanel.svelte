...
<div class="sort-panel relative">
	...
	{#if showSortMenu}
		<div
			class="bg-popover/80 text-popover-foreground absolute top-full right-0 z-50 mt-1 min-w-[180px] rounded-md border py-1 shadow-lg backdrop-blur-md"
		>
			{#each sortOptions as option}
				...
				>
					...
					<SortIconComponent
						class="h-4 w-4 {sortField === option.field ? 'text-primary' : 'text-muted-foreground'}"
					/>
				</button>
			{/each}
			...
			<div class="px-3 py-2">
				...
				<div class="flex gap-2">
					<button
						class="flex-1 rounded px-2 py-1 text-xs transition-colors {sortOrder === 'asc'
					...
					<button
						class="flex-1 rounded px-2 py-1 text-xs transition-colors {sortOrder === 'desc'
							? 'bg-primary text-primary-foreground'
							: 'bg-muted text-muted-foreground'}"
						onclick={() => {
