...
>
	<Dialog.Content class="max-w-lg">
		...
		<div class="space-y-4 py-4">
			...
			{#if emmTags.length > 0}
				<div class="space-y-2">
					<h4 class="text-muted-foreground text-sm font-medium">EMM 标签</h4>
					<div class="flex flex-wrap gap-1.5">
						{#each emmTags as tag}
							<span
								class="inline-flex items-center gap-1 rounded border px-2 py-0.5 text-xs {getNamespaceColor(
									tag.namespace
								)}"
			...
			<!-- 手动标签 -->
			<div class="space-y-2">
				<h4 class="text-muted-foreground text-sm font-medium">手动标签</h4>
				{#if isLoading}
					<p class="text-muted-foreground text-sm">加载中...</p>
				{:else if manualTags.length === 0}
					<p class="text-muted-foreground text-sm">暂无手动标签</p>
				{:else}
					<div class="flex flex-wrap gap-1.5">
						{#each manualTags as tag}
							...
							<span
								...
								title="{NAMESPACE_LABELS[tag.namespace] || tag.namespace}: {tag.tag}{translated !==
								tag.tag
									? ` (${translated})`
									: ''}"
							>
				...
				<div class="flex items-center justify-between">
					...
					{#if isBatchMode}
						<div class="flex items-center gap-2">
							<span class="text-muted-foreground text-xs">已选 {paths.length} 个文件</span>
							<Button
								variant={operationMode === 'add' ? 'default' : 'destructive'}
								size="sm"
								class="h-7 px-2 text-xs"
								onclick={() => {
									operationMode = operationMode === 'add' ? 'remove' : 'add';
								}}
							>
				...
				<div class="flex gap-2">
					...
					<div class="relative flex-1">
						<Input
							...
							onfocus={() => {
								if (newTag.trim()) searchSuggestions(newTag);
							}}
							onblur={() => {
								setTimeout(() => {
									showSuggestions = false;
						...
						{#if showSuggestions && suggestions.length > 0}
							<div
								class="bg-popover absolute top-full right-0 left-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-md border shadow-lg"
							>
								{#each suggestions as suggestion, index}
									<button
										type="button"
										class="hover:bg-accent flex w-full items-center gap-2 px-3 py-2 text-left text-sm {index ===
									...
									>
										<span
											class="rounded px-1.5 py-0.5 text-xs {getNamespaceColor(
