<script lang="ts">
	...
	function syncFromSettings() {
		...
		console.log('[SidebarFloatingControl] syncFromSettings:', { ctrl, viewSettings: s.view });
		enabled = ctrl?.enabled ?? true;
		position = ctrl?.position ?? { x: 100, y: 100 };
		console.log('[SidebarFloatingControl] enabled:', enabled, 'position:', position);
	}
...
{#if enabled}
	...
	>
		<div
			class="border-border/60 bg-background/80 flex items-center gap-0.5 rounded-lg border p-1 shadow-lg backdrop-blur-sm"
		>
			<!-- 拖拽手柄 -->
			<button
				type="button"
