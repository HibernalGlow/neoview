...
export class PageDataRepository {
	...
	private async loadPage(pageIndex: number, loader: () => Promise<Blob>): Promise<PageData> {
		const data: PageData = {
			pageIndex,
			blob: new Blob(),
			url: '',
	...
	private async processDecodeQueue(): Promise<void> {
		while (this.decodingCount < this.options.preDecodeConcurrency && this.decodeQueue.length > 0) {
