...
import type { PreloadTask, PreloadConfig, LoadStatus, VirtualPage } from './types';
...
export class PreloadPipeline {
	...
	private canStartTask(type: PreloadTask['type']): boolean {
		const running = Array.from(this._taskQueue.values()).filter(
			(t) => t.status === 'loading' && t.type === type
		);
