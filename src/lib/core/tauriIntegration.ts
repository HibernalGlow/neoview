 ...
 * 从文件系统加载图像
 */
export async function loadImageFromFile(filePath: string, signal?: AbortSignal): Promise<Blob> {
	// 检查是否已取消
	if (signal?.aborted) {
...
export function createUpscaler(model: string = 'realesrgan-x4plus-anime', scale: number = 2) {
	return async (virtualPage: VirtualPage, imageBlob: Blob, signal: AbortSignal): Promise<Blob> => {
...
export async function openBookWithTauri(
	...
	// 获取文件列表
	const files = isArchive ? await listArchiveImages(path) : await listImageFiles(path);
