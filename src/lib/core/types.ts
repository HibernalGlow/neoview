...
export function pageRangeContains(range: PageRange, position: PagePosition): boolean {
	...
	return (
		comparePagePosition(range.min, position) <= 0 && comparePagePosition(position, range.max) <= 0
	);
}
...
export const defaultPreloadConfig: PreloadConfig = {
	...
	maxConcurrentThumbnails: Math.min(
		32,
		Math.max(16, (typeof navigator !== 'undefined' ? navigator.hardwareConcurrency || 4 : 4) * 4)
	),
