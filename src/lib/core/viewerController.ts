...
import type { ViewState, ViewerConfig, ViewMode, FitMode, Size, Point } from './types';
...
export class ViewerController {
	...
	zoomTo(scale: number, center?: Point, animate: boolean = false): void {
		const newScale = Math.max(this._config.minScale, Math.min(this._config.maxScale, scale));
	...
	private animationTick(): void {
		...
		this._state.offset = {
			x: this.lerp(this._animationStartState.offset.x, this._animationTargetState.offset.x, eased),
			y: this.lerp(this._animationStartState.offset.y, this._animationTargetState.offset.y, eased)
		};
