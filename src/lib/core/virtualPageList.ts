...
import type { PhysicalPage, VirtualPage, VirtualPageListConfig, Rect, PagePosition } from './types';
...
export class VirtualPageList {
	...
	updatePhysicalPageSize(physicalIndex: number, width: number, height: number): boolean {
		...
		const needsSpecialHandling =
			page.isLandscape && page.aspectRatio > this._config.divideThreshold;
	...
	rebuild(): void {
		...
		for (const physical of pages) {
			...
			if (this.shouldDivide(physical)) {
				...
				// 根据阅读方向决定顺序
				const parts =
					this._config.readOrder === 'rtl'
						? [
								{ part: 1 as const, crop: rightCrop },
								{ part: 0 as const, crop: leftCrop }
							]
						: [
								{ part: 0 as const, crop: leftCrop },
								{ part: 1 as const, crop: rightCrop }
	...
	private filterPages(pages: PhysicalPage[]): PhysicalPage[] {
		...
		return pages.filter((page) => {
			return (
				page.entryName.toLowerCase().includes(keyword) || page.path.toLowerCase().includes(keyword)
			);
		});
	}
