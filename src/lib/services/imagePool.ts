...
class ImagePoolImpl {
	...
	private preloadQueue: Map<
		string,
		{ priority: 'high' | 'normal' | 'low'; abortController: AbortController }
	> = new Map();
	...
	private async processPreloadQueue(): Promise<void> {
		...
		// 按优先级排序
		const sorted = Array.from(this.preloadQueue.entries()).sort((a, b) =>
			this.comparePriority(b[1].priority, a[1].priority)
		);
		...
		for (const [key, { priority, abortController }] of sorted) {
			...
			this.loadWithAbort(key, priority, abortController.signal).finally(() => {
