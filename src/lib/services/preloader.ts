...
class PreloaderImpl {
	...
	updateQueue(currentPage: number): void {
		...
		if (deltaPage === 0) {
			...
			const sameDirection =
				(deltaPage > 0 && this.direction === 'forward') ||
				(deltaPage < 0 && this.direction === 'backward');
		...
		// 计算需要预加载的页面
		const pagesToPreload = this.calculatePreloadPages(currentPage, this.totalPages, this.direction);
