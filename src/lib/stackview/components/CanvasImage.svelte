...
<script lang="ts">
	...
	let pendingUrl = '';
	// 【关键】标记当前 bitmap 是否为共享的（来自缓存），如果是则不应关闭
	let isSharedBitmap = false;

	// 获取显示 URL（优先超分图，响应式）
	let displayUrl = $derived.by(() => {
		const version = imagePool.version;
		const hasUpscaled = imagePool.hasUpscaled(pageIndex);
		return hasUpscaled ? (imagePool.getUpscaledUrl(pageIndex) ?? url) : url;
	});
	...
	async function loadAndRender(imageUrl: string, imageBlob?: Blob) {
		...
		try {
			// 1. 尝试从缓存获取预解码的 Bitmap
			// 注意：仅当没有使用超分图时才使用预解码缓存
			const hasUpscaled = imagePool.hasUpscaled(pageIndex);
			if (!hasUpscaled) {
				const cachedBitmap = stackImageLoader.getCachedBitmap(pageIndex);
				if (cachedBitmap) {
					// 渲染缓存的 Bitmap
					renderBitmap(cachedBitmap, imageUrl, true);
					return;
				}
			}
			...
			// 渲染新解码的 Bitmap
			renderBitmap(result.bitmap, imageUrl, false);
		} catch (error) {
			// 只有当前加载才显示错误
			if (pendingUrl === imageUrl) {
	 ...
	 * @param isShared 是否为共享 Bitmap（来自缓存）
	 */
	function renderBitmap(bitmap: ImageBitmap, imageUrl: string, isShared: boolean) {
		...
		const bgColor = computeBackgroundColorFromBitmap(bitmap, imageUrl);
		if (bgColor) {
		...
		// 释放旧的 bitmap（如果是独占的）
		if (currentBitmap && !isSharedBitmap) {
			currentBitmap.close();
		}

		currentBitmap = bitmap;
		isSharedBitmap = isShared;
