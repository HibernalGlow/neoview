...
<script lang="ts">
	...
	} from '$lib/stores/translation/translationStore.svelte';
	...
	let { pageIndex, imageWidth, imageHeight, scale = 1 }: Props = $props();
	...
	>
		<defs>
			...
			<filter id="text-bg-filter" x="-5%" y="-5%" width="110%" height="110%">
				<feFlood flood-color="rgba(0,0,0,0.7)" result="bg"></feFlood>
				<feMerge>
...
<style>
	...
	.translation-text {
		...
		text-shadow:
			...
			calc(-1 * var(--stroke-width, 0)) calc(-1 * var(--stroke-width, 0)) 0
