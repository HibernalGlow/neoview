...
class UpscaleStore {
	...
	async setEnabled(enabled: boolean) {
		...
		try {
			...
			const { autoUpscaleEnabled, saveSettings } = await import(
	...
	async triggerCurrentPageUpscale() {
		...
		for (
			let i = Math.max(0, pageIndex - preloadRange);
			i <= Math.min(book.pages.length - 1, pageIndex + preloadRange);
			i++
		) {
			...
			if (page) {
				...
				const imagePath = isArchive ? `${bookPath} inner=${page.path}` : page.path; // 如果是文件夹模式，page.path 应该是完整路径
	...
	async triggerProgressiveUpscale(currentPageIndex: number, maxPages: number) {
		...
		for (let i = startPage; i < endPage; i++) {
			...
			if (page) {
				...
				const imagePath = isArchive ? `${bookPath} inner=${page.path}` : page.path;
