 ...
 * 将一张横向图分成两个虚拟页
 */
export function applySplitToImage(image: FrameImage, splitHalf: 'left' | 'right'): FrameImage {
	return {
		...image,
		splitHalf,
		// 虚拟页索引需要调整
		virtualIndex: splitHalf === 'left' ? image.physicalIndex * 2 : image.physicalIndex * 2 + 1
	};
}
 ...
 * 横向图旋转90度
 */
export function applyAutoRotate(image: FrameImage, size: ImageSize): FrameImage {
	if (isLandscape(size)) {
		return {
			...image,
			rotation: 90
 ...
 * @returns 每个元素对应的缩放比例
 */
function calculateContentScales(sizes: ImageSize[], stretchMode: WidePageStretch): number[] {
 ...
 */
export function shouldSplitPage(page: PageData, divideLandscape: boolean): boolean {
