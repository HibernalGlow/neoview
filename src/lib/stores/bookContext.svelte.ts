...
export class BookContext {
	...
	toggleOrientation() {
		this._viewState.orientation =
			this._viewState.orientation === 'horizontal' ? 'vertical' : 'horizontal';
	}
	...
	restoreMemory(memory: BookMemory) {
		...
		if (memory.viewState) {
			...
			if (memory.viewState.panoramaEnabled !== undefined)
				this._viewState.panoramaEnabled = memory.viewState.panoramaEnabled;
			if (memory.viewState.orientation) this._viewState.orientation = memory.viewState.orientation;
			if (memory.viewState.zoomLevel !== undefined)
				this._viewState.zoomLevel = memory.viewState.zoomLevel;
			if (memory.viewState.rotation !== undefined)
				this._viewState.rotation = memory.viewState.rotation;
		}
