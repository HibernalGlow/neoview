...
function loadBookmarks(): Bookmark[] {
	try {
		...
		if (stored) {
			...
			return parsed
				.filter(
					(b): b is Partial<Bookmark> & { path: string } =>
				...
				.map((b) => {
					...
					const normalizedListIds =
						Array.isArray(b.listIds) && b.listIds.length > 0
							? Array.from(
									new Set(b.listIds.filter((id) => typeof id === 'string' && id.length > 0))
								)
...
const { subscribe: subscribeActiveList, set: setActiveList } = writable<BookmarkListFilter>(
	loadActiveListId(initialLists)
);
...
function sanitizeListIds(listIds: string[] | undefined): string[] {
	...
	const sanitized = source.map((id) => String(id)).filter((id) => existing.has(id));
...
function upsertBookmark(bookmarks: Bookmark[], item: FsItem, listIds: string[]): Bookmark[] {
	const targetListIds = sanitizeListIds(listIds);
	const existingIndex = bookmarks.findIndex(
		(bookmark) => normalizePath(bookmark.path) === normalizePath(item.path)
	);
...
export const bookmarkStore = {
	...
	clearByFolder(folderPath: string) {
		...
		update((bookmarks) => {
			const newHistory = bookmarks.filter(
				(b) => !normalizePath(b.path).startsWith(normalizedTarget)
			);
	...
	async cleanupInvalid(): Promise<number> {
		...
		// 过滤出存在的条目
		const validBookmarks = checkResults.filter((r) => r.exists).map((r) => r.bookmark);
