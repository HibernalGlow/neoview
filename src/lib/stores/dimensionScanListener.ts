...
export async function initDimensionScanListener(): Promise<void> {
	...
	// 监听扫描进度事件
	progressUnlisten = await listen<DimensionScanProgress>('dimension-scan-progress', (event) => {
	...
	// 监听扫描完成事件
	completeUnlisten = await listen<DimensionScanComplete>('dimension-scan-complete', (event) => {
