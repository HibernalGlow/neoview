...
import type { FrameSnapshot, FrameImageInfo, CropRect, SplitHalf, FrameLayoutType } from '$lib/api/frameApi';
...
class ReaderStore {
  ...
  async loadWindow(
    ...
    // 跳过相同页（非强制）
    if (!force && this.state.currentPage === centerPage && this.state.currentFrame && !this.state.loading) {
      return;
    }
    ...
    try {
      ...
      if (radius > 0) {
        ...
        // 中心帧也设置（用于尺寸计算等）
        const center = fixedFrames.find(f => f.pageIndex === centerPage) ?? fixedFrames[0] ?? null;
        if (center) {
  ...
  /** 上报视口尺寸 */
  async reportViewportSize(width: number, height: number, dpr: number, viewMode: 'single' | 'double' | 'panorama'): Promise<void> {
    try {
