	...
	try {
		...
		if (meta.manualTranslationDictPath) {
			writeStringToLocalStorage(
				'neoview-emm-translation-dict-path',
				meta.manualTranslationDictPath
			);
		}
...
function importPanelsLayout(panelsLayout: any): void {
	...
	try {
		...
		if (panelsLayout.sidebarManagement)
			writeJsonToLocalStorage('neoview-sidebar-management', panelsLayout.sidebarManagement);
		if (panelsLayout.sidebarConfig)
			writeJsonToLocalStorage('neoview-sidebar-config', panelsLayout.sidebarConfig);
	} catch (error) {
...
function importExtendedSettings(extended: any, modules: ImportOptions['modules']): void {
	...
	if (modules.fileBrowserSort && extended.folderPanelSettings) {
		...
		if (fps.quickFolderTargets)
			writeJsonToLocalStorage('neoview-quick-folder-targets', fps.quickFolderTargets);
	}
	...
	if (modules.panelsLayout && extended.virtualPanelSettings) {
		if (extended.virtualPanelSettings.history) {
			writeJsonToLocalStorage(
				...
				extended.virtualPanelSettings.history
			);
		}
		if (extended.virtualPanelSettings.bookmark) {
			writeJsonToLocalStorage(
				'neoview-bookmark-panel-settings',
				extended.virtualPanelSettings.bookmark
			);
		}
	}
