...
export type ThumbnailSource =
  ...
  | { kind: 'archiveEntry'; archivePath: string; innerPath: string; entryIndex: number; fileSize: number }
  | { kind: 'directoryCover'; dirPath: string; representative: string; fileSize: number; modified: number }
...
export function generateThumbKey(source: ThumbnailSource, maxSize: number): string {
  ...
  switch (source.kind) {
    case 'file':
      ...
      break;
    case 'archiveEntry':
      parts.push('A', source.archivePath, source.innerPath, `${source.entryIndex}`, `${source.fileSize}`);
      break;
    case 'directoryCover':
      parts.push('D', source.dirPath, source.representative, `${source.fileSize}`, `${source.modified}`);
      break;
    case 'bookPage':
      parts.push('B', source.bookPath, `${source.pageIndex}`, source.pagePath, `${source.fileSize}`);
      break;
  }
