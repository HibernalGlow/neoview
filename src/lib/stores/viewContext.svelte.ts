...
import { isContainer, inferContentType, createRef } from '$lib/types/content';
...
class ViewContextStore {
	...
	private async openContainer(container: ContentItem): Promise<void> {
		const items = await contentApi.expandContainer(container.path, container.innerPath, 'name');
	...
	private async openLeafItem(item: ContentItem): Promise<void> {
		...
		const index = items.findIndex(
			(i: ContentItem) => i.path === item.path && i.innerPath === item.innerPath
		);
	...
	private emitNavigationEvent(): void {
		...
		if (typeof window !== 'undefined') {
			window.dispatchEvent(
				new CustomEvent('viewcontext:navigate', {
					detail: {
	...
	async restoreFromHistory(pathStack: ContentRef[], currentIndex: number): Promise<void> {
		...
		try {
				...
				if (isLast && !isContainerPath) {
					...
					if (frame) {
						const index = frame.items.findIndex(
							(item: ContentItem) => item.path === ref.path && item.innerPath === ref.innerPath
						);
						if (index >= 0) {
			...
			if (this.currentFrame) {
				this.currentFrame.currentIndex = Math.min(currentIndex, this.currentFrame.items.length - 1);
			}
