...
function enhanceColor(hex: string): string {
	...
	const q =
		enhancedL < 0.5 ? enhancedL * (1 + enhancedS) : enhancedL + enhancedS - enhancedL * enhancedS;
 ...
 * 批量预加载调色板
 */
export async function preloadPalettes(sources: string[], options?: PaletteOptions): Promise<void> {
