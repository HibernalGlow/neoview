...
function keepLuminance(
	...
	const _s = s - 1;
	const p =
		1 * (Math.abs(r - center) * 0.2126) +
		1 * (Math.abs(g - center) * 0.7152) +
		1 * (Math.abs(b - center) * 0.0722);
 ...
 * 生成 SVG 滤镜的 feFunc 值
 */
export function generateColorizeFeFunc(colors: ColorPoint[]): {
	feFuncR: string;
	feFuncG: string;
	feFuncB: string;
} {
