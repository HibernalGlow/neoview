...
export class BatchThumbnailLoader {
	 ...
	 * 智能批量加载（自动选择模式）
	 */
	async smartLoad(paths: string[], options: BatchLoadOptions = {}): Promise<Map<string, string>> {
		const { batchSize = this.DEFAULT_BATCH_SIZE, incremental = false, onBatchReady } = options;
