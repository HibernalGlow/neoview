 ...
 */
export function isAngleInSector(angle: number, startAngle: number, sweepAngle: number): boolean {
...
export function distributeAngles(
	...
	return Array.from({ length: count }, (_, i) => startAngle + step * i + step / 2);
}

/**
 * Get the closest item index based on pointer angle
 */
export function getClosestItemIndex(pointerAngle: number, itemAngles: number[]): number {
