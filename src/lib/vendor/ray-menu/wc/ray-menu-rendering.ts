...
export function createBubble(
	...
	const pos = toCartesian({ x: centerX, y: centerY }, { angle, distance: placementRadius });
...
export function adjustBubbleSubmenuFanAngle(
	...
	const testAngles = distributeAngles(childCount, parentAngle - fanSweep / 2, fanSweep);
 ...
 */
export function createConnector(x1: number, y1: number, x2: number, y2: number): SVGLineElement {
...
export function renderBubbleParentLevels(
	...
	const rootAngles = distributeAngles(rootItems.length, config.startAngle, config.sweepAngle);

	const placementRadius = config.radius * 0.72;
	const maxBubbleRadius = Math.min(
		placementRadius * Math.sin(Math.PI / Math.max(rootItems.length, 1)) * 0.85,
		(config.radius - config.innerRadius) / 2.0
	);
	const bubbleRadius = Math.min(maxBubbleRadius, 48);

	const selectedRootIndex = rootItems.findIndex((i) => i.id === rootEntry.item.id);
	...
	for (let stackIdx = 0; stackIdx < navStack.length - 1; stackIdx++) {
		...
		const selectedChildIndex = childItems.findIndex((i) => i.id === nextEntry.item.id);
		...
		svg.appendChild(
			createConnector(entry.parentCenter.x, entry.parentCenter.y, connectorEnd.x, connectorEnd.y)
		);
