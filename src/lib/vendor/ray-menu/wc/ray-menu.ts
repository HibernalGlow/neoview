...
export class RayMenu extends BaseElement {
		...
		switch (name) {
			...
			case 'start-angle':
				...
				this._config.startAngle =
					newValue !== null ? (Number(newValue) * Math.PI) / 180 : DEFAULT_CONFIG.startAngle;
				break;
			case 'sweep-angle':
				// Convert degrees to radians, default is 360 (full circle)
				this._config.sweepAngle =
					newValue !== null ? (Number(newValue) * Math.PI) / 180 : DEFAULT_CONFIG.sweepAngle;
				break;
			case 'static':
	...
	private _calculateBubbleHoveredIndex(pointer: Point): number {
		...
		const inRootRingBand =
			distFromCenter >= this._config.innerRadius && distFromCenter <= this._config.radius * 1.3;
		...
		if (this._config.infiniteSelection && this._config.infiniteThreshold > 0) {
			const distToParent = distance(parentScreen, pointer);
	...
	private _calculateRootBubbleHoveredIndex(pointer: Point): number {
		...
		const rootItems = this._navStack.length > 0 ? this._navStack[0].items : this._currentItems;
		const itemAngles = distributeAngles(
			rootItems.length,
			this._config.startAngle,
			this._config.sweepAngle
	...
	private _performSubmenuEntry(item: MenuItem, entryAngle: number, confirmedEntry: boolean): void {
		...
		if (this._variant === 'bubble') {
			...
			const parentIndex = this._currentItems.findIndex((i) => i.id === item.id);

			let parentCenter: { x: number; y: number };
			let parentAngle: number;

			if (this._navStack.length > 0 && this._navStack[this._navStack.length - 1].parentCenter) {
				...
				parentAngle = parentIndex >= 0 ? rootAngles[parentIndex] : entryAngle;
	...
	private _clearContainer(): void {
		if (!this.shadowRoot) return;
		this.shadowRoot.querySelectorAll('.ray-frost-wrapper').forEach((el) => el.remove());
		const container = this.shadowRoot.querySelector('.ray-menu-container');
		if (container) {
			container.remove();
		}
	...
	private _updateSelectionState(): void {
		...
		const isBubbleSubmenu = this._variant === 'bubble' && this._navStack.length > 0;
		const activeLevel = isBubbleSubmenu ? String(this._navStack.length) : null;

		const arcs = this.shadowRoot.querySelectorAll('.ray-menu-arc, .ray-menu-bubble');
		let activeIndex = 0;
		arcs.forEach((arc) => {
	...
	private _render(): void {
		...
		const isBubbleSubmenu = this._variant === 'bubble' && this._navStack.length > 0;
		const radius = isBubbleSubmenu ? this._config.radius : this._getCurrentRadius();
		const innerRadius = isBubbleSubmenu ? this._config.innerRadius : this._getCurrentInnerRadius();
		const itemAngles = distributeAngles(
			this._currentItems.length,
			this._config.startAngle,
			this._config.sweepAngle
		...
		if (this._variant === 'bubble') {
			if (this._navStack.length > 0) {
				...
				renderBubbleParentLevels(svg, container, this._navStack, this._config, svgCenter);
				...
				if (deepestEntry.parentCenter && deepestEntry.submenuAngles) {
					...
					this._currentItems.forEach((item, index) => {
						...
						const isHovered = index === this._hoveredIndex;
						const isFocused = index === this._focusedIndex && this._keyboardActive;
						circle.setAttribute('data-hovered', String(isHovered || isFocused));
						...
						circle.setAttribute('data-level', String(this._navStack.length));
				...
				const maxBubbleRadius = Math.min(
					placementRadius * Math.sin(Math.PI / Math.max(itemCount, 1)) * 0.85,
...
if (typeof customElements !== 'undefined' && !customElements.get('ray-menu')) {
